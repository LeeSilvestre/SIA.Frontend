<template>
  <v-app>
    <v-content>
      <v-container class="center-container" fluid>
        <v-row align="center" justify="center" >
          <v-col cols="12" sm="8" md="8">
            <v-card class="elevation-12">
              <v-window v-model="step">
                <v-windown-item value="1">
                  <v-row>
                    <v-col cols="12" md="8">
                      <v-card-text class="mt-1">
                        <div class="text-center mb-3">
                            <v-img src="..\assets\img\schoolLogo.png" max-height="150" max-width="150" class="mx-auto"></v-img>
                        </div>
                        <h2 class="text-center mlt-5">
                          Welcome to  <span class="text-indigo-darken-4">SNA!</span>
                        </h2>
                        <h4 class="text-center mb-4" >
                          Registrar Panel
                        </h4>
                        <v-form>
                          <v-text-field 
                            label="Email/ID"
                            name="Email"
                            prepend-icon="mdi-email"
                            type="text"
                            color="indigo darken-4" />
                          <v-text-field 
                            id="password"
                            label="Password"
                            name="Password"
                            prepend-icon="mdi-lock"
                            type="password"
                            color="indigo darken-4" />
                        </v-form> 
                      </v-card-text>
                      <div class="text-center pb-3">
                        <v-btn rectangle color="#11235A" dark> <router-link to="/dashboard" style="color:white; text-decoration: none;">Sign in</router-link></v-btn>
                      </div>
                      <div class="text-center pb-3">
                        <a href="" style="display: inline-block;">
                          St. Nicholas Academy General Privacy Notice
                        </a>
                      </div>
                    </v-col>
                    <v-col cols="12" md="4" class="rectangle-container">
                      <v-card-text class="white--text mt-4 pt-16">
                      <div class="text-container text-center">
                        <h1 class="display-1 font-weight-bold">ST. NICHOLAS ACADEMY OF CASTILLEJOS INC.</h1>
                        <h5 class="display-5 font-weight-light">CASTILLEJOS, ZAMBALES</h5>
                      </div>
                      <div class="logo-container">
                        <!-- Logo positioned at the right side -->
                        <v-img src="..\assets\img\schoolLogo3.png" max-height="800" max-width="800" class="logo"></v-img>
                      </div>
                    </v-card-text>
                    </v-col>
                  </v-row>
                </v-windown-item>
                <v-windown-item value="2">

                </v-windown-item>
              </v-window>
            </v-card>
          </v-col>
        </v-row>
      </v-container>
    </v-content>
  </v-app>
</template>
<script>
  import ('@/assets/js/login.js');
</script>

<style> 
@import url('@/assets/css/admin_login.css');
</style>


