<template>
  ...
  <tr>
    ...
    <td style="text-align: center;">{{ item.first_name }} {{ item.middle_name }} {{ item.last_name }} {{ item.extension }}</td>
    ...
    <td :style="{ color: getStatusColor(item.enrollment_status), textAlign: 'center' }">{{ item.enrollment_status }}</td>
    <td style="text-align: center;">
      <v-btn color="blue-darken-1" variant="text" @click="close">Edit</v-btn>
      <v-btn color="blue-darken-1" variant="text" @click="save">Enroll</v-btn>
    </td>
  </tr>
</template>
...
export default {
  ...
  computed: {
  formTitle() {
    return this.editedIndex === -1 ? 'ENROLL NEW STUDENT' : 'Edit Student Information';
  },
  displayedStudents() {
    const searchTerm = this.search.toLowerCase();
    return this.students.filter(student =>Object.values(student)
      .some(value =>(value === 'Assessed')
  ));}
},
