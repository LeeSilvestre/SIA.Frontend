<template>
  <v-data-table :search="search" :headers="headers" :items="displayedStudents"
    ...
    <template v-slot:top>
      <v-toolbar flat>
        <v-toolbar-title class="text-h6 font-weight-black " style="color: #2F3F64">STUDENT ADMISSION
        ...
        <v-dialog v-model="dialog" max-width="1000px">
          <template v-slot:activator="{ props }">
            <v-btn class="mb-2 rounded-l	border" color="primary" dark v-bind="props" prepend-icon="mdi-plus">Add New
              Student</v-btn>
          </template>

          <v-card>
            <div style="background-color: var(--dark);">
              <v-card-title><span class="text-h5 fw-bold m-2" style="color: white">{{ formTitle }}</span></v-card-title>
            </div>
            <v-card-text>
              ...
              <v-container>
                <v-row>
                  ...
                  <v-col cols="12" md="3" sm="6" v-if="['8', '9', '10', '11', '12'].includes(editedItem.grade_level)">
                    <div class="mt-2">
                      <span>You are a transferee.</span>
                    </div>
                  </v-col>
                  <v-col cols="12" md="3" sm="6" v-if="['11', '12'].includes(editedItem.grade_level)">
                    <v-select v-model="editedItem.strand" :items="['STEM', 'ABM', 'HUMSS', 'GAS']" label="Select Strand"
                      required></v-select>
                  </v-col>
                </v-row>
                <v-row dense>
                  ...
                  <v-col cols="12" md="3" sm="6">
                    <v-text-field v-model="editedItem.city" label="City/Municipality" required></v-text-field>
                  </v-col>
